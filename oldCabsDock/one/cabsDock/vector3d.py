        ...
        if args:
            if len(args) == 3:
                ...
                if len(words) == 3:
                    ...
                    raise ValueError('Invalid string format to initialize Vector3d object.')
                ...
                if v.shape == (1, 3):
                    ...
                    raise ValueError('Invalid matrix shape: ' + str(v))
            elif len(args) == 1 and type(args[0]) == tuple:
                self.x = args[0][0]
                self.y = args[0][1]
                self.z = args[0][2]
            else:
                raise ValueError('Invalid number or arguments passed to initialize Vector3d object.')
        elif kwargs:
            for arg in kwargs:
                if arg in ['x', 'y', 'z']:
                    self.__dict__[arg] = float(kwargs[arg])
                else:
                    raise ValueError('Vector3d init: invalid option: %s' % arg)
